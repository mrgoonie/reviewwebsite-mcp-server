...
import reviewWebsiteTools from './tools/reviewwebsite.tool.js';
...
indexLogger.debug('ReviewWebsite MCP server module loaded');
...
export async function startServer(mode: 'stdio' | 'http' = 'stdio') {
	...
	serverLogger.info(`Initializing ReviewWebsite MCP server v${VERSION}`);
	...
	reviewWebsiteTools.register(serverInstance);
	serverLogger.debug('Registered ReviewWebsite tools');
